...
def main():
    ...
    )  # /Users/kacper/Desktop/NYPD_project/data/alcohol_data.csv
    fire_incidents_data = dp.load_excel_file(
        os.path.join(input_dir, "fire_incidents.xlsx"), 0
    )  # /Users/kacper/Desktop/NYPD_project/data/fire_incidents.xlsx
    population_data = dp.load_excel_file(
        os.path.join(input_dir, "population_data.xls"), 8
    )  # /Users/kacper/Desktop/NYPD_project/data/area_data.xlsx
    area_data = dp.load_excel_file(
        os.path.join(input_dir, "area_data.xlsx"), 0
    )  # /Users/kacper/Desktop/NYPD_project/data/population_data.xls
    ...
    alcohol_vs_population = dp.custom_merge(
        alcohol_data, population_data, "Miejscowość", "Nazwa jednostki"
    )
    corr_alc_vs_pop = alcohol_vs_population["Count"].corr(
        alcohol_vs_population["Populacja"]
    )
    ...
    corr_all_fire_vs_alc = fire_vs_alcohol["Count"].corr(
        fire_vs_alcohol["Ogółem zdarzeń rok 2024"]
    )
    corr_actual_fire_vs_alc = fire_vs_alcohol["Count"].corr(
        fire_vs_alcohol["Pożary rok 2024"]
    )
    corr_false_fire_vs_alc = fire_vs_alcohol["Count"].corr(
        fire_vs_alcohol["Alarmy fałszywe rok 2024"]
    )

    # FIRE INCIDENTS VS POPULATION
    fire_vs_population = dp.custom_merge(
        fire_incidents_data, population_data, "TERYT", "Kod"
    )
    corr_all_fire_vs_pop = fire_vs_population["Populacja"].corr(
        fire_vs_population["Ogółem zdarzeń rok 2024"]
    )
    corr_actual_fire_vs_pop = fire_vs_population["Populacja"].corr(
        fire_vs_population["Pożary rok 2024"]
    )
    corr_false_fire_vs_pop = fire_vs_population["Populacja"].corr(
        fire_vs_population["Alarmy fałszywe rok 2024"]
    )
    ...
    results = pd.DataFrame(
        {
            "correlation": [
                "Correlation between population and number of alcohol selling companies: ",
    ...
    results.to_csv(
        os.path.join(output_dir, "Results.txt"), sep="\t", index=False
    )
